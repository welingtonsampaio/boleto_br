  ...
  module Strategies
    class HSBC < BoletoBr::Strategies::Commom
      ...
      def setup
        ...
        @documento            = String.new @data[:numero_documento]
        @vencimento           = Date.new @data_venc


        #// número do documento (sem dvs) é 13 digitos
        @nnum = formata_numero @data[:numero_documento],13,0
        #// nosso número (com dvs) é 16 digitos
        @nosso_numero = gera_nosso_numero @nnum, @codigocedente, @vencimento,'4'
        ...
        @linha = barra.slice(0,4) . $dv . barra.slice(4)
